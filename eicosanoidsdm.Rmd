...
( mvars <- tribble(~var, ~type, ~dropmissing, ~useforfiltering,
                   ...
                   "pregnant", "binomial", FALSE, TRUE,
                   "PREVAL_DIAB_T1", "binomial", FALSE, TRUE,
                   "INCIDENT_DIAB_T1", "binomial", FALSE, TRUE,
                   "INCIDENT_CR_ANYCANC", "binomial", FALSE, TRUE,
                   "PREVAL_CR_ANYCANC", "binomial", FALSE, TRUE) %>%
...
<details><summary>Number of missing values in participants</summary>

```{r filtered values missing, echo = FALSE}
dset.raw %>%
    select(any_of(getmissing(mvars))) %>%
    gather(key, value) %>%
    group_by(key) %>%
    summarize(n = n(),
...
<details><summary>Number of excluded participants</summary>

```{r filtered values excluded, echo = FALSE}
dset.raw %>%
    select(any_of(getfilters(mvars))) %>%
    gather(key, value) %>%
    group_by(key) %>%
    summarize(n = n(),
              n_na = sum(is.na(value)),
...
dset %>% 
    dplyr::select(one_of(getcontinuous(mvars))) %>%
    gather(key, value) %>%
    group_by(key) %>%
    summarize(n = n(),
              na = sum(is.na(value)),
...
```{r cox check proportionality, eval = FALSE}
cox.dm %>%
    lastelement %>%
